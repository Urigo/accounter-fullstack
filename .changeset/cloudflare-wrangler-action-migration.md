...
The publish step now uses `cloudflare/wrangler-action` running `wrangler pages deploy`. `wrangler` is
pinned as an exact devDependency of the client, matching the version `email-ingestion-gateway`
already uses, so the action reuses the workspace installation instead of installing one into a Yarn
Berry workspace during CI.
