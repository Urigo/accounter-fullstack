...
async function seed() {
  ...
  try {
    ...
    //
    // `ON CONFLICT DO NOTHING` keeps this rerunnable. The id is derived rather than generated by
    // the database, so without it a second run would fail on the primary key -- and this script
    // runs outside a transaction, so a partial failure is resumed by rerunning it. The self-owning
    // UPDATE below normalizes `owner_id` either way.
    await client.query(
      `
    ...
    console.log('✅ Ensured admin business entity');

    // Create corresponding business record. `owner_id` is NOT NULL with no default, and the admin
    // business owns itself, so it is the entity id.
    await client.query(
      `
    ...
    console.log('✅ Ensured corresponding business record');
