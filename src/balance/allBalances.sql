...
--         ,now()::timestamp
    ...
    FROM accounter_schema.exchange_rates
     ...
     this_month_private_creditcard AS (
         SELECT event_date,
                SUM(event_amount_in_usd_with_vat_if_exists)
                OVER (ORDER BY event_date, event_number) as sum_till_this_point,
                event_number
         FROM local_formatted_merged_tables
         WHERE account_type = 'creditcard'
     ...
     dotan_dept AS (
         SELECT financial_accounts_to_balance,
                event_date,
                event_amount,
                currency_code,
                event_amount_in_usd,
     ...
     new_business_account_transactions AS (
         SELECT event_date,
                ...
                OVER (ORDER BY event_date, event_number, event_amount, bank_reference, account_number)          as dotan_sum_till_this_point,
                financial_entity,
                user_description,
                SUM(case
                ...
                OVER (ORDER BY event_date, event_number, event_amount, bank_reference, account_number)          as sum_vat,
                vat,
                event_vat_amount_in_usd,
                bank_reference,
                account_number,
                event_number
         FROM local_formatted_merged_tables
         WHERE (account_number = 1082 OR account_number = 466803 OR account_number = 1074)
     ...
     current_vat_status AS (
         SELECT event_date,
                ...
                financial_entity
         FROM local_formatted_merged_tables
         WHERE (account_number = 2733 OR account_number = 61066) AND
