...
export default {
  name: '2024-07-25T14-10-36.enhance-view-tables.sql',
  run: ({ sql }) => sql`
  ...
  as
  WITH original_transaction AS (
    SELECT isracard_creditcard_transactions.id::text                         AS raw_id,
  ...
  SELECT DISTINCT ON (t.id) t.id,
    ...
    CASE
      WHEN original_transaction.origin = 'ISRACARD'::text AND
        t.currency = 'ILS'::accounter_schema.currency AND
        t.debit_date IS NULL AND t.debit_date_override IS NULL THEN alt_debit_date.event_date
  ...
  FROM accounter_schema.transactions t
    ...
    LEFT JOIN original_transaction ON original_transaction.raw_id = COALESCE(rt.creditcard_id::text,
    ...
    LEFT JOIN alt_debit_date
      ON alt_debit_date.reference_number = original_transaction.card_number AND alt_debit_date.event_date > t.event_date AND
        alt_debit_date.event_date < (t.event_date + '40 days'::interval);
`,
