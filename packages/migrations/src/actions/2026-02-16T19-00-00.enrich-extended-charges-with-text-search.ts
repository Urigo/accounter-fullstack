...
export default {
  name: '2026-02-16T19-00-00.enrich-extended-charges-with-text-search.sql',
  run: ({ sql }) => sql`
create or replace view accounter_schema.extended_charges
            (id, owner_id, is_property, accountant_status, user_description, created_at, updated_at, tax_category_id,
             ...
             documents_optional_flag, merged_search_text)
as
WITH years_of_relevance AS (SELECT charge_spread.charge_id,
     ...
     transactions_by_charge AS (SELECT transactions.charge_id,
                                       ...
                                       string_agg(COALESCE(transactions.source_description, '') || ' ' || COALESCE(transactions.source_reference, ''), ' ') AS search_text
     ...
     documents_by_charge AS (SELECT documents.charge_id,
                                    ...
                                    string_agg(COALESCE(documents.description, '') || ' ' || COALESCE(documents.remarks, '') || ' ' || COALESCE(documents.serial_number, ''), ' ') AS search_text
...
SELECT c.id,
       ...
       LOWER(
           CONCAT_WS(' ', 
               c.user_description, 
               transactions_by_charge.search_text, 
               documents_by_charge.search_text
           )
