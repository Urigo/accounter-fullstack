...
export default {
  name: '2024-09-01T13-30-24.business-trip-transactions-view-fix.sql',
  run: ({ sql }) => sql`
        ...
        WITH transactions_by_business_trip_transaction AS (SELECT tm.business_trip_transaction_id,
                                                                ...
                                                                sum(tm.amount - COALESCE(misc.amount, 0::numeric)) AS amount,
                                                        ...
                                                        FROM accounter_schema.business_trips_transactions_match tm
                                                                ...
                                                                LEFT JOIN (SELECT authorities_misc_expenses.transaction_id,
                                                                                sum(authorities_misc_expenses.amount) AS amount
                                                                        FROM accounter_schema.authorities_misc_expenses
                                                                        GROUP BY authorities_misc_expenses.transaction_id) misc
        ...
        SELECT DISTINCT ON (btt.id) btt.id,
                                ...
                                CASE
                                        ...
                                        END                                AS value_date,
                                CASE
                                        WHEN t.business_trip_transaction_id IS NULL
                                        THEN (CASE WHEN ep.amount IS NULL THEN NULL ELSE ep.amount * -1 END)
