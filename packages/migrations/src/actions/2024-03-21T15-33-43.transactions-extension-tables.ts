...
export default {
  ...
  (
      id           uuid                  not null
          constraint transactions_fees_pk
              primary key
          constraint transactions_fees_transactions_id_fk
  ...
  -- create transactions bank deposits table
  create table accounter_schema.transactions_bank_deposits
  (
      id           uuid                  not null
          constraint transactions_bank_deposits_pk
              primary key
          constraint transactions_bank_deposits_transactions_id_fk
              references accounter_schema.transactions,
      deposit_id text
  );

  -- update extended_transactions view
          ...
          LEFT JOIN (SELECT base.charge_id,
                      ...
                      FROM (SELECT b_1.charge_id,
                            ...
                            FROM (SELECT transactions.charge_id,
                                        ...
                                        CASE WHEN transactions_fees.id IS NULL THEN FALSE ELSE TRUE END AS is_fee
  ...
  -- update transaction trigger functions
  ...
  -- drop fee column on transactions table
