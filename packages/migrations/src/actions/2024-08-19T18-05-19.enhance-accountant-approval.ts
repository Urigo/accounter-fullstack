...
export default {
  name: '2024-08-19T18-05-19.enhance-accountant-approval.sql',
  run: ({ sql }) => sql`
    create type accounter_schema.accountant_status as enum ('UNAPPROVED', 'APPROVED', 'PENDING');

    alter table accounter_schema.charges
      add accountant_status accounter_schema.accountant_status default 'UNAPPROVED'::accounter_schema.accountant_status not null;

    alter table accounter_schema.business_trips
      add accountant_status accounter_schema.accountant_status default 'UNAPPROVED'::accounter_schema.accountant_status not null;

    update accounter_schema.charges
    set accountant_status = 'APPROVED'::accounter_schema.accountant_status
    where accountant_reviewed is true;
    ...
    create or replace view accounter_schema.extended_business_trips
                (id, name, destination, trip_purpose, from_date, to_date, accountant_status) as
