...
export default {
  name: '2024-08-04T12-37-14.add-document-type-other.sql',
  run: ({ sql }) => sql`
    CREATE TYPE accounter_schema.document_type_new AS ENUM ('INVOICE', 'RECEIPT', 'INVOICE_RECEIPT', 'PROFORMA', 'UNPROCESSED', 'CREDIT_INVOICE', 'OTHER');
    ...
    ALTER TABLE accounter_schema.documents
          ALTER COLUMN type TYPE accounter_schema.document_type_new
            USING (type::text::accounter_schema.document_type_new);

    ALTER TABLE accounter_schema.documents
          ALTER COLUMN type SET DEFAULT 'UNPROCESSED'::accounter_schema.document_type_new;

    DROP TYPE accounter_schema.document_type;

    ALTER TYPE accounter_schema.document_type_new RENAME TO document_type;

    create or replace view accounter_schema.extended_charges
                (id, owner_id, is_property, accountant_reviewed, user_description, created_at, updated_at, tax_category_id,
                ...
                years_of_relevance, invoice_payment_currency_diff, type)
    as
