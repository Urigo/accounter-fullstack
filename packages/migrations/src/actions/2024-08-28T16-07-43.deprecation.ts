...
export default {
  name: '2024-08-28T16-07-43.deprecation.sql',
  run: ({ sql }) => sql`
        create table accounter_schema.deprecation_categories
        (
        ...
        create unique index deprecation_categories_id_uindex
                on accounter_schema.deprecation_categories (id);

        alter table accounter_schema.deprecation_categories
                add constraint deprecation_categories_pk
                        primary key (id);

        create type accounter_schema.deprecation_types as enum ('MARKETING', 'RESEARCH_AND_DEVELOPMENT', 'GENERAL_AND_MANAGEMENT');

        create table accounter_schema.deprecation
        (
                id              uuid default gen_random_uuid() not null
                        constraint deprecation_pk
                        primary key,
                charge_id       uuid                           not null
                        constraint deprecation_charges_id_fk
                        references accounter_schema.charges,
                amount          numeric,
                currency        accounter_schema.currency,
                activation_date date                           not null,
                type            accounter_schema.deprecation_types,
                category        uuid                           not null
                        constraint deprecation_deprecation_categories_id_fk
                        references accounter_schema.deprecation_categories
        );

        create index deprecation_activation_date_index
                on accounter_schema.deprecation (activation_date);

        create index deprecation_charge_id_index
                on accounter_schema.deprecation (charge_id);

        create unique index deprecation_id_uindex
                on accounter_schema.deprecation (id);
        ...
        FROM accounter_schema.charges c
                ...
                LEFT JOIN accounter_schema.deprecation d ON d.charge_id = c.id;
`,
