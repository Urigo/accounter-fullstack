...
export default {
  ...
  run: ({ sql }) => sql`
    alter table accounter_schema.transactions
        add debit_date_override date;
        
    -- must drop the view for adding new column; must drop extended-charges to drop extended-transactions
    ...
    create or replace view accounter_schema.extended_transactions
                (id, charge_id, business_id, currency, debit_date, debit_timestamp, source_debit_date, event_date, account_id, account_type,
    ...
    create or replace view accounter_schema.extended_charges
                ...
                ledger_count, ledger_financial_entities, years_of_relevance, invoice_payment_currency_diff)
    as
    SELECT c.id,
        c.owner_id,
        c.is_conversion,
