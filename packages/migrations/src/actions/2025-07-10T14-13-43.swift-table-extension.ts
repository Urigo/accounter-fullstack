...
export default {
  name: '2025-07-10T14-13-43.swift-table-extension.sql',
  run: ({ sql }) => sql`
alter table accounter_schema.poalim_swift_account_transactions
    add swift_instruction_code_23e text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_exchange_rate_36 text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_account_with_institution_57 text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_senders_to_receiver_information_72 text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_regulatory_reporting_77b text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_ordering_customer_50k text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_ordering_institution_52d text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_beneficiary_customer_59 text;

alter table accounter_schema.poalim_swift_account_transactions
    add swift_remittance_information_70 text;
