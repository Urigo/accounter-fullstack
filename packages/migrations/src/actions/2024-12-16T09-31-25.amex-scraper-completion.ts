...
export default {
  name: '2024-12-16T09-31-25.amex-scraper-completion.sql',
  run: ({ sql }) => sql`
    create trigger amex_transaction_insert_trigger
        after insert
        on accounter_schema.amex_creditcard_transactions
        for each row
    execute procedure accounter_schema.insert_amex_transaction_handler();
