...
export default {
  name: '2024-07-15T16-22-59.documents-table-reorder.sql',
  run: ({ sql }) => sql`
    alter table accounter_schema.documents
      add charge_id uuid;

    alter table accounter_schema.documents
      drop constraint documents_charges_id_fk;

    alter table accounter_schema.documents
        add constraint documents_charges_id_fk
            foreign key (charge_id) references accounter_schema.charges;
    
    drop index accounter_schema.documents_charge_id_new_index;

    create index documents_charge_id_index
        on accounter_schema.documents (charge_id);

    UPDATE accounter_schema.documents
    SET charge_id = charge_id_new
    WHERE charge_id_new IS NOT NULL;
    ...
    LEFT JOIN (SELECT count(DISTINCT l2.id)                                             AS ledger_count,
      ...
      alter table accounter_schema.documents
        add constraint documents_financial_entities_id_fk
          foreign key (creditor_id) references accounter_schema.financial_entities;
      
      alter table accounter_schema.documents
        add constraint documents_financial_entities_id_fk_2
          foreign key (debtor_id) references accounter_schema.financial_entities;
`,
