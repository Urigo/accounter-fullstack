...
export default {
  name: '2024-09-05T15-51-41.depreciation.sql',
  run: ({ sql }) => sql`
    create table accounter_schema.depreciation_categories
    (
    ...
    create unique index depreciation_categories_id_uindex
        on accounter_schema.depreciation_categories (id);

    alter table accounter_schema.depreciation_categories
        add constraint depreciation_categories_pk
            primary key (id);

    create type accounter_schema.depreciation_types as enum ('MARKETING', 'RESEARCH_AND_DEVELOPMENT', 'GENERAL_AND_MANAGEMENT');

    create table accounter_schema.depreciation
    (
        id              uuid default gen_random_uuid() not null
            constraint depreciation_pk
                primary key,
        charge_id       uuid                           not null
            constraint depreciation_charges_id_fk
                references accounter_schema.charges,
        amount          numeric,
        currency        accounter_schema.currency,
        activation_date date                           not null,
        type            accounter_schema.depreciation_types,
        category        uuid                           not null
            constraint depreciation_depreciation_categories_id_fk
                references accounter_schema.depreciation_categories
    );

    create index depreciation_activation_date_index
        on accounter_schema.depreciation (activation_date);

    create index depreciation_charge_id_index
        on accounter_schema.depreciation (charge_id);

    create unique index depreciation_id_uindex
        on accounter_schema.depreciation (id);
    ...
    INSERT INTO accounter_schema.depreciation_categories(id, name, percentage)
    SELECT *
    FROM accounter_schema.deprecation_categories;

    INSERT INTO accounter_schema.depreciation(id, charge_id, amount, currency, activation_date, type, category)
    SELECT id, charge_id, amount, currency, activation_date, type::text::accounter_schema.depreciation_types, category
    FROM accounter_schema.deprecation;

    DROP TABLE accounter_schema.deprecation;

    DROP TYPE accounter_schema.deprecation_types;

    DROP TABLE accounter_schema.deprecation_categories;
`,
