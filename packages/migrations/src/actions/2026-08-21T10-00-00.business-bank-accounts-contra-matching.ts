...
export default {
  name: '2026-08-21T10-00-00.business-bank-accounts-contra-matching.sql',
  run: ({ sql }) => sql`
-- Keep each business' bank accounts (a business may have several) so that incoming
-- bank transactions can be matched to a local business by their contra (counterparty)
-- account. Modeled as a dedicated link table rather than columns on businesses, to
-- support multiple accounts per business.
CREATE TABLE accounter_schema.businesses_bank_accounts (
...
COMMENT ON TABLE accounter_schema.businesses_bank_accounts IS
    'Bank accounts belonging to a business, used to match incoming bank transactions to the counterparty business by their contra account.';

ALTER TABLE accounter_schema.businesses_bank_accounts
    ENABLE ROW LEVEL SECURITY;

ALTER TABLE accounter_schema.businesses_bank_accounts
    FORCE ROW LEVEL SECURITY;

CREATE POLICY tenant_isolation ON accounter_schema.businesses_bank_accounts
    FOR ALL
    USING (owner_id = accounter_schema.get_current_business_id())
    WITH CHECK (owner_id = accounter_schema.get_current_business_id());

-- One business owns a given (bank, branch, account) at most once within an owner scope.
CREATE UNIQUE INDEX businesses_bank_accounts_dedup_uindex
    ON accounter_schema.businesses_bank_accounts (owner_id, bank_number, branch_number, account_number);

CREATE INDEX businesses_bank_accounts_business_id_idx
    ON accounter_schema.businesses_bank_accounts (business_id);

CREATE TRIGGER set_updated_at
    BEFORE UPDATE ON accounter_schema.businesses_bank_accounts
    FOR EACH ROW
...
as
$$
    SELECT bba.business_id
    FROM accounter_schema.businesses_bank_accounts bba
    WHERE p_owner_id IS NOT NULL
      AND p_account_number IS NOT NULL
      AND p_account_number <> 0
      AND bba.owner_id = p_owner_id
      AND bba.bank_number = p_bank_number
      AND bba.branch_number = p_branch_number
      AND bba.account_number = p_account_number
    LIMIT 1;
$$;
