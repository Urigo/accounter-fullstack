...
export const DocumentsTableCreditorFieldsFragmentDoc = graphql(`
...
type Props = {
  data: FragmentOf<typeof DocumentsTableCreditorFieldsFragmentDoc>;
  refetchDocument: () => void;
};

export const Creditor = ({ data, refetchDocument }: Props): ReactElement => {
  const { get } = useUrlQuery();
  const document = readFragment(DocumentsTableCreditorFieldsFragmentDoc, data);
  const dbCreditor = 'creditor' in document ? document.creditor : undefined;

  const isError =
    !dbCreditor?.id || (document.documentType && ['UNPROCESSED'].includes(document.documentType));
