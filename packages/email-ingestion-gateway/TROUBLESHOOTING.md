...
| `UPSTREAM_ERROR`     | gateway `server-client.ts`                | The server **answered and said no**: a terminal 4xx (400/401/403/404/413, not retried), an exhausted 408/425/429 (retried first), or HTTP 200 carrying GraphQL `errors[]`. Not transient — investigate the server. A 5xx is `TRANSIENT_UPSTREAM`, not this. |
...
2. **Body → PDF**, only when **no business is recognized** _or_ `config.emailBody === true`, and the
   ...
   blocked and the Playwright waits are capped at `RENDER_TIMEOUT_MS`, so a render failure is not a
   slow or hanging remote asset in the body.
