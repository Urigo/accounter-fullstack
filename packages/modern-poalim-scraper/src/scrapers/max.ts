...
function getAllMonthMoments(startMoment: Moment | string, futureMonths?: number) {
...
async function fetchData(page: Page, options: ScraperOptions) {
...
function getLoginOptions(page: Page, credentials: ScraperSpecificCredentials): LoginOptions {
  return {
    loginUrl: LOGIN_URL,
...
async function login(
  page: Page,
  credentials: ScraperSpecificCredentials): Promise<ScraperScrapingResult> {
  if (!credentials || !page) {
...
async function initialize(options: ScraperOptions) {
...
options: ScraperOptions): Promise<ScraperScrapingResult> {
  ...
  let loginResult;
  try {
    loginResult = await login(page, credentials);
  } catch (e) {
    loginResult =
