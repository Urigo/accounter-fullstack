...
describe('SHAAM Format Round-trip Integration Test', () => {
  it('should generate and parse back a complete report', () => {
    ...
    expect(parseInt(parsedData.closingRecord?.totalRecords || '0')).toBe(expectedTotalRecords - 2); // Z900 counts data records only (excludes A000 and Z900 itself)
  ...
  it('should handle empty data sections correctly', () => {
    ...
    expect(result).toBeDefined();
    expect(result.dataText).toBeDefined();
    expect(result.summary.totalRecords).toBe(3); // A000 + A100 + Z900
