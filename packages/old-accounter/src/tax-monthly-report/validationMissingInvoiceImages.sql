...
DROP FUNCTION missing_invoice_images (month_input VARCHAR);

CREATE OR REPLACE FUNCTION missing_invoice_images (month_input VARCHAR) RETURNS SETOF accounter_schema.all_transactions LANGUAGE SQL AS $$
...
WHERE
      this_month_business.proforma_invoice_file IS NULL AND
