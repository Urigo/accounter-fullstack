...
/**
 * Asserts the `Account.Execution[]` payload before `poalimSecuritiesTransactionsVars`
 * maps it onto the upload mutation. The object is `strictObject`, so an unmodelled
 * bank-side field fails here instead of reaching the mutation unchecked, and every
 * constraint carries a message naming the field, the offending value and the fix.
 *
 * This mirrors `HapoalimSecuritiesTransactionsSchema` in
 * `@accounter/modern-poalim-scraper` on purpose: the scraper validates what the bank
 * sent, this validates what we are about to upload. Keep the two in step —
 * `__tests__/validate-payload.test.ts` covers this copy.
 *
 * Field names are the bank's own, misspellings included (`IsraeTaxValue`,
...
/**
 * A closed set of labels the bank sends. Extending one of these lists is the
 * intended fix when the message below fires — the values are the bank's
 * vocabulary, not ours.
 */
function poalimEnum<const T extends readonly [string, ...string[]]>(
  ...
  return z.enum(values, {
    error: issue =>
      `${field}: unexpected value ${describeInput(issue.input)}. Known values are ${quoted(values)} — if the bank started sending another, add it to ${constant} (here and in the scraper's schema).`,
  });
}
...
/**
 * `.NET` round-trip timestamps: `2026-08-11T00:00:00.0000000+03:00`. Only the
 * shape is enforced — `ExecutionDate` uses the offset-less
 * `0001-01-01T00:00:00.0000000` sentinel, hence the optional offset.
 */
...
function alwaysNull(field: string) {
  return z.null({
    error: issue =>
      `${field}: was null in every observed response and is typed as null, but this one carries ${describeInput(issue.input)}. Give the field a real type here, in the scraper's schema, in PoalimSecurityTransactionInput and in the poalim_securities_transactions column.`,
  });
}
...
const ExecutionFields = z.strictObject({
  // ── identity (the fields the upload dedup key is built from) ────────────────
  ...
  // ── account ─────────────────────────────────────────────────────────────────
  // Branch/Account are asserted but not uploaded: the vars mapper stamps the
  // requested account instead, and the cross-field check below proves they agree.
 ...
 */
const ExecutionItemSchema = ExecutionFields.superRefine((row, ctx) => {
  ...
  // half-filled block would silently break the upload dedup key, which is
  ...
  // The vars mapper stamps the requested account onto every row rather than
  // reading these; if they disagree, the upload would file the execution under
  // an account it did not happen in.
  ...
  // one, and the uploaded amounts are only comparable while that holds.
  if (row.TradeCurrency !== row.SettlementCurrency) {
    ctx.addIssue({
      code: 'custom',
      path: ['SettlementCurrency'],
      message: `SettlementCurrency "${row.SettlementCurrency}" differs from TradeCurrency "${row.TradeCurrency}" — a cross-currency settlement needs an explicit conversion before upload.`,
    });
  }
  ...
  // Both name fields carry the same text; if they ever diverge we are storing
  // one language in two columns and losing the other.
...
export const PoalimSecuritiesTransactionsPayloadSchema = z.strictObject({
  Account: z.strictObject({
    PageState: z
      ...
      .nullable()
      .optional(),
    // A portfolio with no activity in the range omits this entirely.
    Execution: z
      .array(ExecutionItemSchema, {
...
export type PoalimSecuritiesTransactionsPayload = z.infer<
  typeof PoalimSecuritiesTransactionsPayloadSchema
>;
