...
export const depreciationReport: ResolverFn<
  ReadonlyArray<ResolversTypes['DepreciationReportResult']>,
  ResolversParentTypes['Query'],
  GraphQLModules.Context,
  Partial<QueryDepreciationReportArgs>
> = async (_, { filters }, { injector }) => {
  if (!filters) {
    throw new GraphQLError('No filters provided');
  }
  const { year, financialEntityId } = filters;
  // years.map(year => {
