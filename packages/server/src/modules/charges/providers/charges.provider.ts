...
import { Injectable, Scope, Inject, CONTEXT } from 'graphql-modules';
import { sql } from '@pgtyped/runtime';
import type { Optional, TimelessDateString, AccounterContext } from '../../../shared/types/index.js';
...
export class ChargesProvider {
  ...
  private get executor() {
    const rlsClient = this.context.rlsClient;
    if (rlsClient) {
      return {
        query: <E extends import('pg').QueryResultRow>(
          queryText: string,
          values: any[],
        ) => {
          return rlsClient
  ...
  public async getChargesByMissingRequiredInfo() {
    return getChargesByMissingRequiredInfo.run(undefined, this.executor).then(charges =>
      charges.map(c => {
  ...
  public updateCharge(params: IUpdateChargeParams) {
    ...
    return updateCharge.run(params, this.executor).then(([newCharge]) => {
      if (newCharge) {
  ...
  public batchUpdateCharges(params: IBatchUpdateChargesParams) {
    {
      ...
      return batchUpdateCharges.run(params, this.executor).then(charges => {
  ...
  public updateAccountantApproval(params: IUpdateAccountantApprovalParams) {
    return updateAccountantApproval.run(params, this.executor).then(([newCharge]) => {
      if (newCharge) {
        this.getChargeByIdLoader.prime(newCharge.id, newCharge);
      }
      return newCharge;
  ...
  public generateCharge(params: IGenerateChargeParams) {
    ...
    return generateCharge.run(fullParams, this.executor).then(([newCharge]) => {
      if (newCharge) {
        this.getChargeByIdLoader.prime(newCharge.id, newCharge);
      }
      return newCharge;
  ...
  public getChargesByFilters(params: IGetAdjustedChargesByFiltersParams) {
    ...
    return getChargesByFilters.run(fullParams, this.executor) as Promise<
      IGetChargesByFiltersResult[]
    >;
  }
  ...
  public async getSimilarCharges(params: IGetSimilarChargesParams) {
    try {
      return getSimilarCharges.run(params, this.executor) as Promise<
        IGetChargesByFiltersResult[]
      >;
    } catch (error) {
  ...
  public deleteChargesByIds(params: IDeleteChargesByIdsParams) {
    return deleteChargesByIds.run(params, this.executor);
  }
