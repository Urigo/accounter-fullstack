...
export async function startServer() {
  ...
  const yoga = createYoga({
    plugins: [
      dbCleanupPlugin(),
      // Add OpenTelemetry plugin if enabled (before auth to trace all operations)
      ...(env.otel?.enabled
        ? [
            useOpenTelemetry({
              resolvers: true,
              variables: true,
              result: false, // Don't include result by default to avoid logging sensitive data, can be enabled for debugging
            }),
          ]
        : []),
      authPlugin(),
  ...
  const gracefulShutdown = async (
    ...
    try {
      // Shutdown OpenTelemetry SDK to flush pending telemetry
      await shutdownOtel();
      // Close the database pool
      await pool.end();
    } catch (e) {
      process.stderr.write(`[shutdown] Error during cleanup: ${String(e)}\n`);
    } finally {
