...
2. Deliver new service in parallel path, then validate in shadow mode.
3. Use test confidence, not strict cutover gates, as the primary readiness signal.
4. Cut over tenant-by-tenant only after passing test suite and shadow checks.
5. Preserve fast rollback by keeping old listener operational until full confidence.
...
1. Gateway package (listener reuse)
...
2. Server authoritative path
   1. packages/server/src/modules/gmail-listener/typeDefs/gmail-listener.graphql.ts
   2. packages/server/src/modules/gmail-listener/resolvers/gmail-listener.resolver.ts
   3. packages/server/src/modules/auth/providers/auth-context.provider.ts
   4. packages/server/src/modules/app-providers/tenant-db-client.ts
