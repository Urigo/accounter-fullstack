...
export function DocumentMatchesSelection({document, charges, setReadyForNextStep}: Props) {
    ...
    if ((document.__typename !== 'Invoice' && document.__typename !== 'Receipt' && document.__typename !== 'InvoiceReceipt' && document.__typename !== 'Proforma') || document.amount?.raw == null || !document.date) {
  ...
  return (
    <div className="relative items-center w-full mx-auto md:px-12 lg:px-2 max-w-8xl">
      <div>
        <div className={`relative p-10 space-y-12 overflow-hidden lg:space-y-0 lg:grid lg:grid-cols-${ratedMatches.length > 2 ? '4' : String(ratedMatches.length+1)} lg:gap-x-8 rounded-xl`}>
          <div className="relative flex flex-col p-8 bg-white">
            <div className="flex-1">
              ...
              <ul className="pt-6 mt-6 space-y-6 border-t">
                ...
                <li></li>
                <li>
                <button onClick={() => setExtandedInfoFlag(false)} className="inline-flex items-center mt-4 font-semibold text-blue-600 lg:mb-0 hover:text-neutral-600" title="read less"> « Less Info </button>
                </li>
                </>) : (<li>
                    <button onClick={() => setExtandedInfoFlag(true)} className="inline-flex items-center mt-4 font-semibold text-blue-600 lg:mb-0 hover:text-neutral-600" title="read more"> More Info » </button>
                </li>)}
              </ul>
            </div>
          </div>
            ...
            <div className="relative flex-1">
              ...
              <ul className="pt-6 mt-6 space-y-6 border-t">
                ...
                <li className="flex">
                  ...
                  <span className="ml-3 text-white">{format(new Date(charge.transactions[0].createdAt), 'yyyy-MM-dd')}</span>
                </li>
                <li className="flex">
                  <div className="inline-flex items-center w-6 h-6 bg-white rounded-xl p-0.5">
                  <Coin color='blue' />
                ...
                <li></li>
                <li>
                <button onClick={() => setExtandedInfoFlag(false)} className="inline-flex items-center mt-4 font-semibold text-white lg:mb-0 hover:text-neutral-300" title="read less"> « Less Info </button>
                </li>
                </>) : (<li>
                    <button onClick={() => setExtandedInfoFlag(true)} className="inline-flex items-center mt-4 font-semibold text-white lg:mb-0 hover:text-neutral-300" title="read more"> More Info » </button>
                </li>)}
