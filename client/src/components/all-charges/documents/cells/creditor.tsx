...
export const Creditor = ({ data }: Props) => {
  const { get } = useUrlQuery();
  const document = getFragmentData(DocumentsTableCreditorFieldsFragmentDoc, data);
  const creditor = 'creditor' in document ? document.creditor : undefined;
  const isError =
    ![
      ...
      DocumentType.Receipt,
    ].includes(document.documentType as DocumentType) && !creditor?.id;
