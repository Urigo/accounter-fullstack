...
import pg from 'pg';
const { Pool } = pg;
function currencyCodeToSymbol(currency_code) {
    let currencySymbol = '₪';
...
export const financialStatus = async () => {
    const pool = new Pool({
        user: 'postgres',
        host: 'localhost',
        database: 'accounter',
    ...
    `;
    const lastInvoiceNumbersQuery = readFileSync('src/sql/lastInvoiceNumbers.sql').toString();
