...
import {
  IGetHashavshevetBusinessIndexesParams,
  IGetHashavshevetBusinessIndexesQuery,
  IGetHashavshevetGovIndexesQuery,
} from '../generated-types/hashavshevet.provider.types.mjs';
...
const getHashavshevetGovIndexes = sql<IGetHashavshevetGovIndexesQuery>`
    SELECT *
...
export class HashavshevetProvider {
  constructor(private pool: Pool, private financialAccounts: FinancialAccountsProvider) {}

  public getHashavshevetIsracardIndex = async (charge: IGetChargesByIdsResult) => {
    if (charge.financial_entity === 'Isracard' && charge.bank_reference) {
      const hashCreditcardIndexResult = await this.financialAccounts.getFinancialAccountByAccountNumberLoader.load(
  ...
  public getHashavshevetVatIndexes = async (ownerId: string) => {
    const indexes = await getHashavshevetGovIndexes.run({ ownerId }, this.pool);
  ...
  public getHashavshevetBusinessIndexes = async (params: IGetHashavshevetBusinessIndexesParams) => {
    return await getHashavshevetBusinessIndexes.run(params, this.pool);
  };
}
